...
class StereoOdometry:
    ...
    def compute_pnp(self, Xk_minus_1: np.ndarray, keypoints1: list, matches: list) -> np.ndarray:
        ...
        print("Xk_minus_1 - in: ", Xk_minus_1.shape)
    ...
    def run(self, p_results: str) -> None:
        ...
        for idx, (image_current, image1) in enumerate(tqdm(self.dataloader)):
            ...
            print("len(filtered_matches2): ", len(filtered_matches2))

            if len(filtered_matches) > self.min_matches:
                # Project points from I,k-1 to I,k and compute PnP to get the pose Tk
                Tk = self.compute_pnp(_3D_point, keypoints_current, filtered_matches2)
            ...
            # Triangulate points from filtered matches
            _3D_point = self.triangulate_points([filtered_matches[i[0]] for i in indices], keypoints_prev, keypoints_current)
            print("3D - out: ", _3D_point.shape)
