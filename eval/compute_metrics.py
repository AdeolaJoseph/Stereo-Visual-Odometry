...
#         command = f"evo_ape kitti {os.path.join(base_path,est_file)} {os.path.join(base_path,gt_file)} -va --plot --plot_mode xz" #--save_results eval/results/{gt_file.split('_')[0]+'.zip'}"
#         try:
#             subprocess.run(command, shell=True, check=True)
#         except subprocess.CalledProcessError as e:
#             print(f"Command failed with error: {e}")
